...
// __dirname at runtime: <root>/app/lib/node/ → three levels up = <root>/
...
export class PtyManager {
    ...
    spawn(id: PtyInstanceId, role: PaneRole, cols: number, rows: number, cwd?: string): void {
        ...
        const instance: PtyInstance = { pty, role, socketPath, onDataEmitter, disposed: false };

        // PTY stdout → emitter (with RGB translation for editor PTYs)
        pty.onData((data: any) => {
    ...
    onData(id: PtyInstanceId, cb: (data: string) => void): { dispose: () => void } | undefined {
        const inst = this.instances.get(id);
        if (!inst) return undefined;
        return inst.onDataEmitter.event(cb);
    }
    ...
    /** Reload config from disk (called after settings are saved). */
    reloadConfig(): void {
        const freshCfg = loadConfig(CONFIG_PATH);
        cfg.nvim.bin = freshCfg.nvim.bin;
        cfg.nvim.configMode = freshCfg.nvim.configMode;
        cfg.nvim.configDir = freshCfg.nvim.configDir;
    }
